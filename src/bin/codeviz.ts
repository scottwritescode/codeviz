...
async function loadCodeViz(): Promise<typeof import('../index')> {
  try {
    ...
    console.error(`\n  Error: ${msg}`);
    console.error('\n  Try reinstalling with: npm install -g @scottwritescode/codeviz\n');
    process.exit(1);
  }
}
...
const nodePatch = parseInt(nodeVersion.split('.')[2] ?? '0', 10);
const supportsNode25 = nodeMajor === 25 && (
  nodeMinor > 9 || (nodeMinor === 9 && nodePatch >= 0)
);

if (!supportsNode25) {
  console.warn(
    '\x1b[33m⚠\x1b[0m  CodeViz requires Node.js >=25.9.0 <26.0.0. Current version: %s.',
    nodeVersion
  );
  console.warn(
    '   Please use Node.js 25.9.0: https://nodejs.org/en/download\n'
  );
}
